...
out = waveforms[node(outp[2])]  # Get output from the final inverter in the cascade.
...
# plt.plot(time * 1000, waveforms[node(outp[0])])
# plt.plot(time * 1000, waveforms[node(outp[1])])
# plt.plot(time * 1000, waveforms[node(outp[2])])
