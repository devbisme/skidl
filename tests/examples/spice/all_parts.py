...
isrc = PULSEI(
    initial_value=0 @ u_mA, pulsed_value=2 @ u_mA, pulse_width=1 @ u_ms, period=2 @ u_ms
...
rl = R(value=4.7 @ u_kOhm)  # load resistor in series with power supply.
vs = PULSEV(
    initial_value=0 @ u_V, pulsed_value=2 @ u_V, pulse_width=1 @ u_ms, period=2 @ u_ms
)  # 1ms ON, 1ms OFF pulses.
...
vs = PULSEV(
    initial_value=0 @ u_V, pulsed_value=5 @ u_V, pulse_width=1 @ u_ms, period=2 @ u_ms
)  # 1ms ON, 1ms OFF pulses.
q["s", "d", "g", "b"] += (
    gnd,
    rd[1],
...
# Get the input source waveform and amplified output at the tranistor's collector.
time = waveforms.time
vin = waveforms[node(vs["p"])]  # Input source voltage.
vout = waveforms[node(q["c"])]  # Transistor's collector voltage.
...
vs = PULSEV(
    initial_value=-5 @ u_V, pulsed_value=5 @ u_V, pulse_width=1 @ u_ms, period=2 @ u_ms
)  # 1ms ON, 1ms OFF pulses.
