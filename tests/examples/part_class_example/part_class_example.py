...
class MicrocontrollerModule(SubCircuit):
    ...
    def __init__(self, name="mcu_module"):
        ...
        self.crystal = Part(
            "Device", "Crystal", value="8MHz", footprint="Crystal_HC49-4H"
        )
...
class LEDDriverModule(SubCircuit):
    ...
    def __init__(self, name="led_driver"):
        ...
        self.resistors = [
            Part("Device", "R", value="330", footprint="R_0603") for _ in range(4)
        ]
        self.transistors = [
            Part("Transistor_BJT", "2N3904", footprint="TO-92") for _ in range(4)
        ]

        # Connect LED drivers
        for i, (led, resistor, transistor, ctrl) in enumerate(
            zip(self.leds, self.resistors, self.transistors, self.control)
        ):
...
def create_hierarchical_circuit():
    ...
    main_connector = Part(
        "Connector_Generic", "Conn_01x06", footprint="PinHeader_1x06_P2.54mm_Vertical"
    )
