...
"""
human_readable 模式下的功能拓扑识别（generic driver + generic MCU）。
与 trunk-aware 布局互斥：matched 时专用布局，否则 apply_trunk_aware_layout。
对外保持 ``from skidl.schematics.topology import ...`` 路径不变（含下划线内部 API）。
"""

from . import common, driver, mcu, mcu_fork
...
for _mod in (common, driver, mcu, mcu_fork):
    for _name, _obj in _mod.__dict__.items():
        if _name.startswith("__"):
            continue
        globals()[_name] = _obj

del _mod, _name, _obj, common, driver, mcu
