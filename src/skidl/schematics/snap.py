...
"""
Post-route snap placement — move 2-pin parts onto IC pins for cleaner schematics.

Backend-agnostic: operates on abstract geometry (Point, Tx, orientation strings).
KiCad-specific rendering (S-expression wires, label suppression) lives in
tools/kicad9/sexp_schematic.py.
"""
...
def snap_two_pin_parts(node):
    """Snap 2-pin parts onto their connected IC pins post-route.

    Pass 1: Snap onto IC pins (parts with >2 pins).
    Pass 2+: Chain onto already-snapped 2-pin parts.
    Pass 3: Stack remaining onto occupied IC pins perpendicular.
    """
...
def _pre_shift_ics(plans, node, snapped):
    """Shift ICs vertically so stagger fans won't overlap."""
    for plan in plans:
        ic = plan["ic_part"]
        ic_deps = set()
        ic_id = id(ic)
