...
def sexp_to_nested_list(s_expr):
    ...
    removing quote marks from quoted strings.
    
    This function converts a string containing an S-expression (a format used by KiCad
    and other CAD tools) into a nested Python list structure. It properly handles
    quoted strings, parentheses, and attempts to convert numeric values to appropriate types.
    
    Args:
        s_expr (str): The S-expression string to parse.
    
    Returns:
        list: A nested list representation of the S-expression with quote marks removed.
    ...
    Raises:
        ValueError: If there are unclosed quotes in the S-expression.
    """
...
@export_to_all
def nested_list_to_sexp(nested_list):
    """
    Convert nested Python lists to an S-expression string.
    
    This function converts a nested list structure into an S-expression string format,
    with non-first elements enclosed in double quotes if they are not already quoted.
    The resulting S-expression is formatted with proper indentation.
    
    Args:
        nested_list (list): A nested list structure representing an S-expression.
    
    Returns:
        str: The corresponding S-expression string, nicely formatted.
    ...
    return prettify_sexp("(" + " ".join(elements) + ")")

@export_to_all
def prettify_sexp(sexp, spaces_per_level=2):
    """
    Format an S-expression string with proper indentation.
    
    This function takes an S-expression string and formats it with indentation
    to make it more readable. It handles parentheses, quoted strings,
    and escaped characters properly.
    
    Args:
        sexp (str): The S-expression string to format.
        spaces_per_level (int, optional): Number of spaces per indentation level. Defaults to 2.
        
    Returns:
        str: The formatted S-expression string with proper indentation.
    """
    ...
    while i < len(sexp):
        ...
        if char == '(':
            ...
            if i > 0:  # Don't add newline before first character.
                result.append('\n')
