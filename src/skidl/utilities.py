...
def sexp_to_nested_list(s_expr):
    ...
    Args:
        s_expr (str): The S-expression to parse.
...
def nested_list_to_sexp(nested_list):
    """
    Convert nested Python lists to an S-expression string, with non-first elements
    enclosed in double quotes if they are not already.
    
    Args:
        nested_list (list): A nested list structure representing an S-expression.
    
    Returns:
        str: The corresponding S-expression string.
    
    Example:
        >>> lists_to_s_expression(['define', ['square', 'x'], ['*', 'x', 'x']])
        '(define (square "x") (* "x" "x"))'
    """
